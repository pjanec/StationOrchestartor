...
namespace SiteKeeper.Master.Model.InternalData
{
    ...
    /// </summary>
    /// <remarks>
    /// Each node action is broken down into one or more node tasks, each targeted at a specific Slave Agent.
    ...
    /// - Unique Task ID (within the scope of an action).
    /// - Reference to the parent NodeAction ID.
    ...
    {
        /// <summary>
        /// Unique identifier for this task, typically unique within the scope of its parent node action.
        /// Could be, for example, "{ActionId}-{NodeName}-{TaskSequence}".
        /// </summary>
        /// <example>"na-deploy-webapp-123-AppServer01-1"</example>
        [Required]
        ...
        /// <param name="actionId">Parent node action ID.</param>
