...
namespace SiteKeeper.Master.Services
{
    ...
    /// </summary>
    public class MasterNLogSetup : IHostedService
    {
        private readonly IServiceProvider _serviceProvider;
        private readonly ILogger<MasterNLogSetup> _logger;
        private UILoggingTarget? _uiLoggingTarget;

        public MasterNLogSetup(IServiceProvider serviceProvider, ILogger<MasterNLogSetup> logger)
        {
        ...
        public Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (config.FindTargetByName("ui-forwarder") is not UILoggingTarget)
            {
                ...
                {
                    Name = "SK-MasterActionId",
                    Layout = "${mdlc:item=SK-MasterActionId}"
                });

                _uiLoggingTarget.ContextProperties.Add(new TargetPropertyWithContext
                {
                    Name = "SK-StageIndex",
                    Layout = "${mdlc:item=SK-StageIndex}"
                });

                _uiLoggingTarget.ContextProperties.Add(new TargetPropertyWithContext
                {
                    Name = "SK-StageName",
                    Layout = "${mdlc:item=SK-StageName}"
                });
