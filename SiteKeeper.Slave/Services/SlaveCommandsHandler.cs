...
namespace SiteKeeper.Slave.Services
{
    ...
    /// </remarks>
    public class SlaveCommandsHandler
    {
        ...
        private readonly IEnumerable<ISlaveTaskHandler> _taskHandlers;

        /// <summary>
        /// Initializes a new instance of the <see cref="SlaveCommandsHandler"/> class.
        /// </summary>
        ...
        public SlaveCommandsHandler(
            ...
            IEnumerable<ISlaveTaskHandler> taskHandlers)
        {
        ...
        {
            _logger.Info($"Handling PrepareForTask: OpId '{prepareInstruction.ActionId}', TaskId '{prepareInstruction.TaskId}', ExpectedType '{prepareInstruction.ExpectedTaskType}'.");
            ...
            {
                ActionId = prepareInstruction.ActionId,
        ...
        {
            _logger.Info($"Handling SlaveTask: TaskId '{instruction.TaskId}', Type '{instruction.TaskType}', OpId '{instruction.ActionId}'.");
            ...
            if (!await concurrentTaskSemaphore.WaitAsync(TimeSpan.FromSeconds(5), CancellationToken.None)) // Use CancellationToken.None for semaphore acquisition
            {
                ...
                await _sendTaskUpdateAsyncCallback(new SlaveTaskProgressUpdate { 
                    ActionId = instruction.ActionId, TaskId = instruction.TaskId, NodeName = _agentName,
            ...
            {
                _logger.Warn($"Task {instruction.TaskId} (OpId {instruction.ActionId}) is already active or failed to add to dictionary. Ignoring new request.");
            ...
            taskContext.ExecutionTask = Task.Run(async () =>
            {
                ...
                NLog.MappedDiagnosticsLogicalContext.Set("SK-ActionId", instruction.ActionId);
                ...
                try
                {
                    ...
                    var reportProgress = ( int progress ) =>
                    {
	                    ...
	                    if( taskContext.ShouldSendProgressUpdate( progress ) )
	                    {
		                    _sendTaskUpdateAsyncCallback( new SlaveTaskProgressUpdate
		                    {
			                    ...
			                    NodeName = _agentName,
			                    Status = NodeTaskStatus.InProgress.ToString(),
                    ...
                    // --- Find the correct handler for the task type ---
                    // A specific handler was found, execute it.
                    var handler = _taskHandlers.FirstOrDefault(h => h.Handles == instruction.TaskType);
                    
					if( handler != null )
					{
						success = await handler.ExecuteTaskAsync( instruction, taskContext, reportProgress, taskSpecificNLogLogger );
                        finalMessage = success
                            ? "Task completed successfully by executive code."
                            : (taskContext.FinalResultJson != null && taskContext.FinalResultJson.Contains("error", StringComparison.OrdinalIgnoreCase)
                                ? "Task reported failure and provided error."
                                : "Task execution failed (no error provided).");
					}
					else
					{
                        // fail if no handler is found ---
                        taskSpecificNLogLogger.Error($"No handler registered for TaskType '{instruction.TaskType}'. The task will fail.");
                        finalMessage = $"Task execution failed: No handler is registered on this agent for the task type '{instruction.TaskType}'.";
                        taskContext.FinalResultJson = JsonSerializer.Serialize(new { error = "UnsupportedTaskType", message = finalMessage });
                        success = false;
					}
                ...
                catch (Exception ex)
                {
                    ...
                    taskContext.FinalResultJson = taskContext.FinalResultJson ?? JsonSerializer.Serialize(new { error = ex.GetType().FullName, message = ex.Message, stackTrace = ex.ToString() });
                }
                finally
                {
                    taskContext.LastStatusUpdateUtc = DateTime.UtcNow;
                    ...
                    taskSpecificNLogLogger.Info($"Task {instruction.TaskId} (Op: {instruction.ActionId}) finished with status {taskContext.CurrentLocalStatus}. Semaphore released. Active tasks: {activeSlaveTasks.Count}");

                    // Clear the MDLC context at the very end of the task's lifecycle.
                    NLog.MappedDiagnosticsLogicalContext.Remove("SK-ActionId");
        ...
        public async Task HandleTaskCancelRequestAsync(string operationId, string taskId, ConcurrentDictionary<string, SlaveTaskContext> activeSlaveTasks)
        {
            ...
            if (activeSlaveTasks.TryGetValue(taskId, out var taskContext))
            {
                ...
                if (taskContext.Instruction.ActionId == operationId)
                {
                        ...
                        taskContext.CurrentLocalStatus != SlaveLocalTaskExecutionStatus.Cancelling) // Not already cancelling
                    {
                        ...
                        await _sendTaskUpdateAsyncCallback(new SlaveTaskProgressUpdate {
                            ActionId = operationId, TaskId = taskId, NodeName = _agentName,
                ...
                } else { _logger.Warn($"Mismatched OperationId for TaskId {taskId} on cancel request. Request OpId: {operationId}, Task OpId: {taskContext.Instruction.ActionId}. Ignoring."); }
        ...
        public async Task HandleAdjustSystemTimeAsync(AdjustSystemTimeCommand command, int maxTimeAdjustmentMinutesWithoutForce)
        {
            ...
            if (Math.Abs(differenceFromCurrent.TotalMinutes) > maxTimeAdjustmentMinutesWithoutForce && !command.ForceAdjustment)
            {
                ...
                await _sendTaskUpdateAsyncCallback(new SlaveTaskProgressUpdate { 
                    ActionId = operationSystemTimeSync, TaskId = taskIdTimeSyncAck, NodeName = _agentName,
